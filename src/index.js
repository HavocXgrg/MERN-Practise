...
// PATCH method to partial(specific0) edit the user collection
app.patch('/users/:id', async (req,res) => {
  try{
    const result = await Users.findByIdAndUpdate(req.params.id, {userName: req.body})
    if(result){
      res.json({msg:`ID ${req.params.id} updated userName successfully`})
    }
  } catch(err){
    console.log(err)
  }
